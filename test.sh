...
tc_client()
{
	...
	tc -n "${ns}" filter add dev "${iface}" egress  bpf da obj tcp_in_udp_tc.o sec tc_client_egress
...
tc_server()
{
	...
	tc -n "${ns}" qdisc add dev "${iface}" clsact
	tc -n "${ns}" filter add dev "${iface}" egress  bpf da obj tcp_in_udp_tc.o sec tc_server_egress
